...
function CamerasPage() {
  ...
  if (loading) return <div className="dark:text-white p-8">Cargando listado...</div>;
        ...
        <table className=" dark:bg-gray-900 min-w-full leading-normal dark:text-gray-300">
          ...
          <tbody>
            {cameras.length === 0 ? (
              ...
              cameras.map((cam) => (
                <tr key={cam.camera_id} className="hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors">
                  <td className="px-5 py-5 border-b border-gray-700 dark:bg-gray-800 text-sm dark:text-white">
                    {cam.camera_id}
                  </td>
                  <td className="px-5 py-5 border-b border-gray-700 dark:bg-gray-800 text-sm dark:text-white">
                  ...
                  <td className="px-5 py-5 border-b border-gray-700 dark:bg-gray-800 text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      cam.current_status === 'online' ? 'bg-green-200 dark:bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {cam.current_status}
                    </span>
                  </td>
